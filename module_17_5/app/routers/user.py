...
async def tasks_by_user_id(db: db_dependency, user_id: int):
    user_tasks = db.scalars(select(Task).where(Task.user_id == user_id)).first()
    return user_tasks
...
async def delete_all_user(db: db_dependency):
    user_delete = db.execute(delete(User))
    db.commit()
