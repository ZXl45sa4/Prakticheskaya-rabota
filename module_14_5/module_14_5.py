...
Inline_kb_0 = InlineKeyboardMarkup()
Inline_button_0 = InlineKeyboardButton(text='Регистрация', callback_data='Registrant')
Inline_kb_0.row(Inline_button_0)
...
async def start(message):
    await message.answer("Привет! Я бот помогающий твоему здоровью.", reply_markup=Inline_kb_0)


@dp.callback_query_handler(text='Registrant')
async def sing_up(message):
    await message.message.answer('Введите имя пользователя (только латинский алфавит):')
...
async def set_username(message, state):
    ...
    if not crud_functions.is_included(username=message.text):
#        print('--- Вы вернулись!')
...
async def set_age(message, state):
    ...
    email1 = data1["email"]
    age1 = data1["age"]
    print(username1, email1, age1)
    crud_functions.add_user(username=username1, email=email1, age=age1)
