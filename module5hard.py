...
class Video:
    ...
    def __init__(self, title, duration, adult_mode = False):
        ...
        self.duration = int(duration)           # продолжительность, сек
        self.time_now = 0                       # остановки, сек
        self.adult_mode = bool(adult_mode)      # ограничение по возрасту
...
class UrTube:
    ...
    """
    Метод log_in, который принимает на вход аргументы: nickname, password и 
    ...
    def log_in(self, nickname, password):
        for user in self.users:
            if user.nickname == str(nickname) and user.password == hash(password):
                self.current_user = user
                return
    """
    Метод register, который принимает три аргумента: nickname, password, age, и 
    ...
    """
    def get_videos(self, PoiskovoyeSlovo):
        PoiskovoyeSlovo = PoiskovoyeSlovo.lower()
        Spisok = []
        for x in self.videos:                       # При помощи цикла for переберите предполагаемо подходящие видео.
            if PoiskovoyeSlovo in x.title.lower():
                Spisok.append(x.title)              # Добавляются видео в результирующий список
                continue
        return Spisok
        ...
        for k in self.videos:
            if k.title == title:
                if k.adult_mode and self.current_user.age > 18:
                    ...
                    return
                for s in range(k.time_now, k.duration): # Значение time_now, duration берется из class Video
                    print(s + 1, end=' ')
                    time.sleep(1)
                print("Конец видео")
                k.time_now = 0
